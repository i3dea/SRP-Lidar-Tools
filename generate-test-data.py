"""USAGE: generate-test-data [options] [--target <target>...]

Generate a test data folder with s sample image and ground truth.

Options:
    -t, --target <target>...            A target object to place in the test image
    -s, --square SIZE                   Add a square target with the given size (in meters)
    -o <folder>, --output <folder>      An output folder [default: ./].
    -m <meters>, --mpp <meters>         The meters per pixel [default: 0.13].
    --nr <num_rects>                    The number of target objects to place [default: 3]
    --width <width>                     The width [default: 512]
    --height <height>                   The height [default: 512]
